...
if (type >= 1)
{
    if (timer == 10)
    {
        ...
        for (var i = 0; i < amount; i++)
        {
            ...
            with (scr_fire_bullet(makex, obj_growtangle.y + (140 * vside), 1407, 180 - (90 * side), 0, 1394))
            {
                target = other.target;
                damage = other.damage;
                vspeed = vside * -(2 - (i / 4));
...
if (make_random_lightning)
{
    ...
    with (scr_fire_bullet(hoz, obj_growtangle.y + (120 * side), 1407, 180 - (90 * side), 0, 1394))
    {
        x += 4;
